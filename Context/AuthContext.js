...
//     router.replace("/WelcomePage"); // Ensure user is redirected to WelcomePage
//   };
...
import React, { createContext, useState, useEffect } from "react";
import { useRouter } from "expo-router"; // Import the hook
import * as SecureStore from "expo-secure-store";
import { View, Text } from "react-native";
import apiClient, { setAuthInterceptor } from "../utils/axiosSetup";
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    // Register the Axios interceptor, passing a function that will be called when the token expires.
    setAuthInterceptor(() => logout);
  ...
  const logout = async () => {
    ...
    // Now router is available so you can redirect
    router.replace("/WelcomePage");
  };

  // Optionally, keep your existing refresh logic if needed
  useEffect(() => {
    const refreshAuthState = async () => {
      const token = await SecureStore.getItemAsync("authToken");
      const storedUserId = await SecureStore.getItemAsync("userId");
